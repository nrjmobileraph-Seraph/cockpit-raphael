...
p = r'C:\Users\BoulePiou\cockpit-raphael\app.py'
with open(p, 'r', encoding='utf-8') as f:
    lines = f.readlines()

NEW_CODE = """if "sidebar_state" not in st.session_state:
...
if st.session_state.get("connected", False) == True:
    ...
    if st.session_state.sidebar_state == "collapsed":
        ...
        div[data-testid="stMainBlockContainer"]>div:first-child div[data-testid="stButton"] button:hover{{{HOVER_CSS}}}
        ...
        section[data-testid="stSidebar"] [data-testid="stSidebarUserContent"]>div:first-child div[data-testid="stButton"] button:hover{{{HOVER_CSS}}}
        </style>''', unsafe_allow_html=True)
        with st.sidebar:
...
# Chercher la ligne de début
for i, line in enumerate(lines):
    if 'if "sidebar_state" not in st.session_state:' in line:
        start_idx = i
        break
...
if start_idx != -1 and end_idx != -1:
    ...
    try:
        ...
        with open(p, 'w', encoding='utf-8') as f:
            f.write(new_src)
        print("OK - Le bloc a ete remplace avec SUCCES !")
    except SyntaxError as e:
        print(f"ERREUR - Syntaxe invalide a la ligne {e.lineno}")
else:
    print("ERREUR - Impossible de trouver les marqueurs de debut ou de fin dans le fichier.")
