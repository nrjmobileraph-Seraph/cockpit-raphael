...
def page_dashboard(profil, cap):
    ...
    # Pioche mensuelle par poche (0 pour l'instant, variables futures)
    _p = [0, 0, 0, 0, 0]  # placements
    _e = [0, 0, 0, 0, 0]  # epargne
    _p_total = sum(_p)
    _e_total = sum(_e)

    # === DASHBOARD VISUEL v20 ===
    dash_html = f'''
<html><head><meta charset="utf-8">
<style>
* {{ margin:0; padding:0; box-sizing:border-box; }}
body {{ background:#0A0508; padding:16px; font-family:'Segoe UI',Arial,sans-serif; color:#F0E6D8; }}
.d {{ max-width:960px; margin:0 auto; position:relative; }}
.box {{
  border:2px solid #8B2525;
  border-radius:10px;
  background: linear-gradient(160deg, #4A1010 0%, #320808 40%, #2A0606 100%);
  text-align:center; padding:16px;
  box-shadow: inset 0 1px 0 rgba(196,146,42,0.08);
}}
.box .lab {{ font-size:11px; color:#C49060; letter-spacing:2px; text-transform:uppercase; }}
.box .val {{ font-size:36px; font-weight:800; color:#F0E6D8; margin:6px 0; }}
.box .sub {{ font-size:12px; color:#906050; }}
.rect {{
  flex:1; border:2px solid #8B2525; border-radius:10px;
  background: linear-gradient(160deg, #3E0C0C 0%, #2C0808 50%, #220606 100%);
  padding:12px 8px;
  box-shadow: inset 0 1px 0 rgba(196,146,42,0.06);
}}
.rect .titre {{ font-size:12px; color:#C49060; letter-spacing:2px; text-transform:uppercase; text-align:center; margin-bottom:8px; }}
.rect .single {{ text-align:center; font-size:14px; font-weight:700; color:#E8B050; }}
.rect .additions {{
  display:grid; grid-template-columns:1fr auto 1fr auto 1fr auto 1fr auto 1fr;
  text-align:center; font-size:12px; font-weight:700; margin-bottom:30px; align-items:center;
}}
.rect .additions .ch {{ color:#E8B050; text-align:center; }}
.rect .additions .pl {{ color:#9B4040; padding:0 2px; }}
.carre {{
  width:70px; min-width:70px; border:2px solid #B04030; border-radius:8px;
  background: linear-gradient(160deg, #3A0C0C 0%, #280606 100%);
  display:flex; align-items:center; justify-content:center;
  font-size:16px; font-weight:700; color:#E8B050;
}}
.poche {{
  border:1px solid #7A2020; border-radius:6px;
  background: linear-gradient(160deg, #350A0A 0%, #220505 100%);
  padding:8px 4px; text-align:center;
}}
.poche .nm {{ font-size:9px; color:#C49060; }}
.poche .vl {{ font-size:11px; font-weight:700; color:#E8B050; margin-top:2px; }}
.sc {{
  flex:1; border:2px solid #8B2525; border-radius:10px;
  background: linear-gradient(160deg, #4A1010 0%, #320808 40%, #2A0606 100%);
  padding:16px; text-align:center;
}}
.sc .sv {{ font-size:28px; font-weight:800; color:#F0E6D8; }}
.sc .sl {{ font-size:11px; color:#906050; margin-top:4px; }}
.zone-fleches {{ position:relative; }}
.zone-fleches svg {{ position:absolute; top:0; left:0; pointer-events:none; z-index:10; }}
#bigsvg {{ position:absolute; top:0; left:0; pointer-events:none; z-index:10; }}
</style></head><body>
<div class="d" id="dash">

<div class="box" id="box-cc">
  <div class="lab">Compte courant</div>
  <div class="val">{_cc_val:,.0f} EUR</div>
  <div class="sub">BoursoBank</div>
</div>
<div style="height:16px;"></div>

<div style="display:flex;gap:16px;">
  <div class="box" style="flex:1;" id="box-80">
    <div class="lab">Budget aujourd'hui</div>
    <div class="val">{_bj} EUR</div>
    <div class="sub">Par jour</div>
  </div>
  <div class="box" style="flex:1;" id="box-1300">
    <div class="lab">Reste ce mois</div>
    <div class="val">{_rm:,} EUR</div>
    <div class="sub">sur {_jr} jours</div>
  </div>
</div>
<div style="height:16px;"></div>

<div style="display:flex;gap:10px;align-items:stretch;">
  <div class="rect" id="rect-rev">
    <div class="titre">Revenus</div>
    <div class="single">{_rev_total:,.0f}</div>
  </div>
  <div class="carre" id="carre-rev">{_rev_total:,.0f}</div>
  <div class="rect" id="rect-rente">
    <div class="titre">Rentes</div>
    <div class="single">0</div>
  </div>
</div>
<div style="height:16px;"></div>

<div class="zone-fleches" id="zone">
  <div style="display:flex;gap:10px;align-items:stretch;">
    <div class="rect" id="rect-plac">
      <div class="titre">Placements</div>
      <div class="additions">
        <div class="ch" id="cg0">{_p[0]}</div><div class="pl">+</div>
        <div class="ch" id="cg1">{_p[1]}</div><div class="pl">+</div>
        <div class="ch" id="cg2">{_p[2]}</div><div class="pl">+</div>
        <div class="ch" id="cg3">{_p[3]}</div><div class="pl">+</div>
        <div class="ch" id="cg4">{_p[4]}</div>
      </div>
    </div>
    <div class="carre" id="carre-plac">{_p_total + _e_total}</div>
    <div class="rect" id="rect-ep">
      <div class="titre">Epargne</div>
      <div class="additions">
        <div class="ch" id="cd0">{_e[0]}</div><div class="pl">+</div>
        <div class="ch" id="cd1">{_e[1]}</div><div class="pl">+</div>
        <div class="ch" id="cd2">{_e[2]}</div><div class="pl">+</div>
        <div class="ch" id="cd3">{_e[3]}</div><div class="pl">+</div>
        <div class="ch" id="cd4">{_e[4]}</div>
      </div>
    </div>
  </div>
  <div style="height:12px;"></div>
  <div style="display:flex;gap:10px;">
    <div style="flex:1;display:grid;grid-template-columns:repeat(5,1fr);gap:6px;" id="pg">
      <div class="poche"><div class="nm">AV1 Cardif</div><div class="vl">{_av1:,.0f} &euro;</div></div>
      <div class="poche"><div class="nm">AV2 Spirit</div><div class="vl">{_av2:,.0f} &euro;</div></div>
      <div class="poche"><div class="nm">AV3 Abeille</div><div class="vl">{_av3:,.0f} &euro;</div></div>
      <div class="poche"><div class="nm">AV4 Bourso</div><div class="vl">{_av4:,.0f} &euro;</div></div>
      <div class="poche"><div class="nm">AV5</div><div class="vl">&mdash;</div></div>
    </div>
    <div style="width:70px;min-width:70px;"></div>
    <div style="flex:1;display:grid;grid-template-columns:repeat(5,1fr);gap:6px;" id="pd">
      <div class="poche"><div class="nm">Livret A</div><div class="vl">{_la:,.0f} &euro;</div></div>
      <div class="poche"><div class="nm">LDDS</div><div class="vl">{_ldds:,.0f} &euro;</div></div>
      <div class="poche"><div class="nm">LEP</div><div class="vl">{_lep:,.0f} &euro;</div></div>
      <div class="poche"><div class="nm">PEA</div><div class="vl">{_pea:,.0f} &euro;</div></div>
      <div class="poche"><div class="nm">Crypto</div><div class="vl">{_crypto:,.0f} &euro;</div></div>
    </div>
  </div>
  <svg id="asvg" xmlns="http://www.w3.org/2000/svg"></svg>
</div>
<div style="height:16px;"></div>

<div style="display:flex;gap:10px;">
  <div class="sc"><div class="sv">2 200</div><div class="sl">Prudent</div></div>
  <div class="sc"><div class="sv">2 500</div><div class="sl">Normal</div></div>
  <div class="sc"><div class="sv">2 700</div><div class="sl">Optimiste</div></div>
</div>

<svg id="bigsvg" xmlns="http://www.w3.org/2000/svg"></svg>
...
function draw(){{
  ...
  svg.setAttribute('width',Math.round(zr.width));
  svg.setAttribute('height',Math.round(zr.height));
  svg.setAttribute('viewBox','0 0 '+Math.round(zr.width)+' '+Math.round(zr.height));
  while(svg.firstChild)svg.removeChild(svg.firstChild);

  function flechesV(pochesId,prefix){{
    var poches=document.getElementById(pochesId).querySelectorAll('.poche');
    for(var i=0;i<poches.length;i++){{
      var p=poches[i].getBoundingClientRect();
      var c=document.getElementById(prefix+i).getBoundingClientRect();
      var x=Math.round(c.left+c.width/2-zr.left);
      var yBas=Math.round(p.top-zr.top);
      var yHaut=Math.round(c.top+c.height+4-zr.top);
      var line=document.createElementNS('http://www.w3.org/2000/svg','line');
      line.setAttribute('x1',x);line.setAttribute('y1',yBas);
      line.setAttribute('x2',x);line.setAttribute('y2',yHaut+10);
      line.setAttribute('stroke','#B04030');line.setAttribute('stroke-width','2');
      svg.appendChild(line);
      var tri=document.createElementNS('http://www.w3.org/2000/svg','polygon');
      tri.setAttribute('points',x+','+yHaut+' '+(x-6)+','+(yHaut+10)+' '+(x+6)+','+(yHaut+10));
      tri.setAttribute('fill','#B04030');
      svg.appendChild(tri);
    }}
  }}
  flechesV('pg','cg');
  ...
  var big=document.getElementById('bigsvg');
  big.setAttribute('width',Math.round(dr.width));
  big.setAttribute('height',Math.round(dr.height));
  big.setAttribute('viewBox','0 0 '+Math.round(dr.width)+' '+Math.round(dr.height));
  while(big.firstChild)big.removeChild(big.firstChild);

  function pos(el){{
    var r=el.getBoundingClientRect();
    return{{cx:Math.round(r.left+r.width/2-dr.left),cy:Math.round(r.top+r.height/2-dr.top),t:Math.round(r.top-dr.top),b:Math.round(r.bottom-dr.top),l:Math.round(r.left-dr.left),r:Math.round(r.right-dr.left)}};
  }}

  function arrow(x1,y1,x2,y2,w,color){{
    var dx=x2-x1,dy=y2-y1;
    var len=Math.sqrt(dx*dx+dy*dy);
    var ux=dx/len,uy=dy/len;
    var px=-uy,py=ux;
    var tLen=w*3,tW=w*2;
    var bx=x2-ux*tLen,by=y2-uy*tLen;
    var line=document.createElementNS('http://www.w3.org/2000/svg','line');
    ...
    var tri=document.createElementNS('http://www.w3.org/2000/svg','polygon');
    tri.setAttribute('points',x2+','+y2+' '+Math.round(bx+px*tW)+','+Math.round(by+py*tW)+' '+Math.round(bx-px*tW)+','+Math.round(by-py*tW));
    tri.setAttribute('fill',color);
    big.appendChild(tri);
  }}

  var cc=pos(document.getElementById('box-cc'));
  var b80=pos(document.getElementById('box-80'));
  var b1300=pos(document.getElementById('box-1300'));
  var cRev=pos(document.getElementById('carre-rev'));
  var rRev=pos(document.getElementById('rect-rev'));
  var rRente=pos(document.getElementById('rect-rente'));
  var cPlac=pos(document.getElementById('carre-plac'));
  var rPlac=pos(document.getElementById('rect-plac'));
  var rEp=pos(document.getElementById('rect-ep'));

  // Petites fleches horizontales
  arrow(rRev.r-10, cRev.cy, cRev.l+4, cRev.cy, 3, '#B04030');
  arrow(rRente.l+10, cRev.cy, cRev.r-4, cRev.cy, 3, '#B04030');
  arrow(rPlac.r-10, cPlac.cy, cPlac.l+4, cPlac.cy, 3, '#B04030');
  arrow(rEp.l+10, cPlac.cy, cPlac.r-4, cPlac.cy, 3, '#B04030');

  // Fleche carre bas vers carre haut
  arrow(cPlac.cx, cPlac.t+12, cRev.cx, cRev.b-16, 3, '#CC5544');

  // Diagonales vers centre du 80 et 1300, s arretent au bord
  var s80x=cRev.cx-20, s80y=cRev.t;
  var d80x=b80.cx-s80x, d80y=b80.cy-s80y;
  var t80=(b80.b-20-s80y)/d80y;
  arrow(s80x, s80y, Math.round(s80x+d80x*t80), b80.b-20, 3, '#C45535');

  var s13x=cRev.cx+20, s13y=cRev.t;
  var d13x=b1300.cx-s13x, d13y=b1300.cy-s13y;
  var t13=(b1300.b-20-s13y)/d13y;
  arrow(s13x, s13y, Math.round(s13x+d13x*t13), b1300.b-20, 3, '#C45535');

  // Grosse fleche vers 22733
  arrow(cRev.cx, cRev.t-2, cc.cx, cc.b-8, 5, '#C49040');
}}
...
window.addEventListener('resize',draw);
    ...
    components.html(dash_html, height=850, scrolling=False)
    ...
    if phase_0:
        ...
        st.markdown(f'<div style="background:linear-gradient(145deg, #0A0505 0%, #1A0A12 100%);border:1px solid #C4922A;border-radius:12px;padding:20px;text-align:center;margin-top:16px;"><div style="color:#BBA888;font-size:12px;text-transform:uppercase;">COMPTE A REBOURS</div><div style="color:#FFD060;font-size:48px;font-weight:900;">{jours_restants} jours</div><div style="color:#DDCCBB;font-size:14px;">avant LMNP operationnel (15 janvier 2027)</div></div>', unsafe_allow_html=True)
        ...
        with k4: kpi("Loyer net",f"{profil['loyer_net']:,.0f} EUR/mois","LMNP reel","bleu")
...
def page_boursobank(profil, cap):
    ...
    st.markdown('<div style="background:#1A0D12;border:2px solid #C4922A;border-radius:12px;padding:20px;margin-bottom:16px;"><div style="color:#FFD060;font-size:18px;font-weight:bold;margin-bottom:12px;">Connexions bancaires</div><div style="color:#BBA888;">Finary agrège tous tes comptes en un seul endroit. Chaque compte connecté remonte automatiquement dans le cockpit.</div></div>', unsafe_allow_html=True)
    titre("Comptes")
    for nom, statut, col in [("BoursoBank (CC)", "Connecté", "#4DFF99"),("Linxea Spirit 2", "A ouvrir", "#FFD060"),("Lucya Cardif", "A ouvrir", "#FFD060"),("Lucya Abeille", "A ouvrir", "#FFD060"),("PEA Bourse Direct", "A ouvrir", "#FFD060"),("Boursorama Vie", "A ouvrir", "#FFD060")]:
        st.markdown(f'<div style="display:flex;justify-content:space-between;background:#140810;border-left:3px solid {col};padding:8px 14px;margin:3px 0;border-radius:4px;"><span style="color:#FFD060;">{nom}</span><span style="color:{col};font-weight:bold;">{statut}</span></div>', unsafe_allow_html=True)
    st.markdown('<div style="margin:16px 0;"><a href="https://app.finary.com" target="_blank" style="display:inline-block;background:#1A6B4B;color:white;padding:10px 24px;border-radius:8px;text-decoration:none;font-weight:bold;">Ouvrir Finary</a></div>', unsafe_allow_html=True)
...
def page_annexe(profil, cap):
    ...
    tableau("Modules", ["Fonction", "Description"], [
        ...
        ["ARVA (Rente Vie Autonome)", "Calcul rente optimale pour 50 000 EUR a 92 ans"],
