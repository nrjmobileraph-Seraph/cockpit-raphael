...
def page_dashboard(profil, cap):
    ...
    if phase_0:
        # === DASHBOARD PHASE 0 REFAIT SELON LE SCHEMA RAPHAEL ===
        import calendar
        ...
        budget_jour = int(round(rail_cible / jours_dans_mois))
        depense_theorique = budget_jour * max(0, today.day - 1)
        reste_mois = max(0, int(round(rail_cible - depense_theorique)))
        jours_restants = jours_dans_mois - today.day + 1

        revenus_items = [
            ("AAH", profil.get("aah_mensuel", 0)),
            ("LMNP", profil.get("loyer_net", 0)),
            ("RVD", profil.get("rvd_mensuel", 0)),
            ("ASPA", profil.get("aspa_mensuelle", 0)),
            ("Pro", profil.get("revenus_pro", 0)),
            ("Autres", profil.get("autres_rentes", 0)),
        ]
        revenus_total = sum(float(v or 0) for _, v in revenus_items)
        complement_capital = max(0, rail_cible - revenus_total)

        poches_items = [
            ("AV1 Cardif", cap.get("av1", 0)),
            ("AV2 Spirit", cap.get("av2", 0)),
            ("AV3 Abeille", cap.get("av3", 0)),
            ("AV4 Bourso", cap.get("av4", 0)),
            ("Livret A", cap.get("livret_a", 0)),
            ("LDDS", cap.get("ldds", 0)),
            ("LEP", cap.get("lep", 0)),
            ("PEA", cap.get("pea", 0)),
            ("Crypto", cap.get("crypto", 0)),
            ("Crowd", cap.get("crowdfunding", 0)),
        ]
        capital_total_affiche = sum(float(v or 0) for _, v in poches_items)

        # Compte courant live si possible
        try:
            ...
            if _tk:
                ...
                _r = _crq.get(
                    "https://api.finary.com/users/me/holdings_accounts",
        ...
        cc_val = float(cap.get("cc", 0) or 0)
        cc_border = "#CC3333" if cc_val < 1000 else ("#D4A017" if cc_val < 2500 else "#1A6B4B")

        mois_restants_total = mois_restants(profil)
        rente_arva = arva(C, mois_restants_total, profil["rendement_annuel"]) if mois_restants_total > 0 else 0
        objectif_capital = 454000
        capital_reel = capital_total_affiche
        pct_objectif = int((capital_reel / objectif_capital) * 100) if objectif_capital > 0 else 0

        prochains = []
        for date_cible, action, flux, sens, solde_prevu, row_id in avenir[:5]:
            try:
                flux_txt = f"{flux:+,.0f}".replace(",", " ")
            except Exception:
                flux_txt = str(flux)
            prochains.append((date_cible, action, flux_txt, sens))
        if not prochains:
            prochains = [("—", "Aucun mouvement saisi", "", "info")]

        objectif_date = date(2027, 1, 15)
        compte_rebours = (objectif_date - today).days

        st.markdown("""
        <style>
        .rp-wrap {display:flex; flex-direction:column; gap:14px; margin: 8px 0 18px 0;}
        .rp-band {
            background: linear-gradient(145deg, #12070C 0%, #1A0D12 50%, #12070C 100%);
            border: 2px solid #C4922A; border-radius: 16px; padding: 18px 22px; text-align:center;
            box-shadow: 0 8px 26px rgba(0,0,0,.28);
        }
        .rp-band .k {color:#BBA888; font-size:12px; letter-spacing:2px; text-transform:uppercase;}
        .rp-band .v {color:#FFD060; font-size:56px; font-weight:900; line-height:1.0; margin-top:6px;}
        .rp-band .s {color:#E8D9C0; font-size:14px; margin-top:6px;}
        .rp-two {display:grid; grid-template-columns:1fr 1fr; gap:14px;}
        .rp-card {
            background: linear-gradient(145deg, #140810 0%, #1B0E15 100%);
            border-radius: 14px; padding: 16px 18px; box-shadow: 0 8px 22px rgba(0,0,0,.18);
        }
        .rp-card.center {text-align:center;}
        .rp-card.green {border:2px solid #1A6B4B;}
        .rp-card.gold {border:2px solid #C4922A;}
        .rp-card.red {border:2px solid #CC3333;}
        .rp-card .k {color:#BBA888; font-size:11px; text-transform:uppercase; letter-spacing:1.6px;}
        .rp-card .v {font-size:34px; font-weight:900; line-height:1.05; margin-top:6px;}
        .rp-card .s {color:#E8D9C0; font-size:12px; margin-top:6px;}
        .rp-node {
            background:#11070C; border:1px solid #7A5B23; border-radius: 999px; padding: 10px 18px;
            text-align:center; max-width: 460px; margin: 0 auto; position: relative;
        }
        .rp-node:before, .rp-node:after {
            content:""; position:absolute; top:50%; width:80px; height:1px; background:#7A5B23;
        }
        .rp-node:before {left:-82px;} .rp-node:after {right:-82px;}
        .rp-node .k {color:#BBA888; font-size:10px; text-transform:uppercase; letter-spacing:1.4px;}
        .rp-node .v {color:#F0E6D8; font-size:16px; font-weight:800;}
        .rp-poles {display:grid; grid-template-columns:1fr 1fr; gap:14px;}
        .rp-pole {
            background: linear-gradient(145deg, #140810 0%, #1A0D12 100%);
            border-radius: 15px; padding: 18px 20px;
        }
        .rp-pole.rev {border:2px solid #1A6B4B;}
        .rp-pole.pioche {border:2px solid #C4922A;}
        .rp-pole .k {color:#BBA888; font-size:12px; text-transform:uppercase; letter-spacing:1.8px;}
        .rp-pole .v {font-size:42px; font-weight:900; line-height:1.0; margin-top:6px;}
        .rp-grid6, .rp-grid10 {display:grid; gap:10px;}
        .rp-grid6 {grid-template-columns: repeat(3, 1fr);}
        .rp-grid10 {grid-template-columns: repeat(5, 1fr);}
        .rp-mini {
            background:#0E0A0E; border-radius:10px; padding:10px 8px; text-align:center;
            min-height:72px; display:flex; flex-direction:column; justify-content:center;
        }
        .rp-mini.rev {border:1px solid #1A6B4B;}
        .rp-mini.cap {border:1px solid #7A5B23;}
        .rp-mini .k {color:#BBA888; font-size:10px; line-height:1.15;}
        .rp-mini .v {font-size:18px; font-weight:900; line-height:1.1; margin-top:4px;}
        .rp-rails {display:grid; grid-template-columns:1.2fr 1fr 1fr 1fr; gap:12px; align-items:stretch;}
        .rp-rail {
            background:#12080E; border-radius:12px; padding:12px 14px; text-align:center;
        }
        .rp-rail.parents {border:2px solid #B35A5A;}
        .rp-rail.plancher {border:2px solid #CC3333;}
        .rp-rail.cible {border:2px solid #D4A017;}
        .rp-rail.bonus {border:2px solid #1A6B4B;}
        .rp-rail .k {color:#BBA888; font-size:10px; text-transform:uppercase; letter-spacing:1.4px;}
        .rp-rail .v {font-size:24px; font-weight:900; margin-top:4px;}
        .rp-bottom {display:grid; grid-template-columns: 1.5fr 1fr; gap:14px;}
        .rp-list {display:flex; flex-direction:column; gap:8px; margin-top:10px;}
        .rp-item {display:flex; justify-content:space-between; gap:12px; padding:9px 10px; border-radius:10px; background:#0D0910; border:1px solid #2C2130;}
        .rp-item .d {color:#BBA888; font-size:11px; min-width:78px;}
        .rp-item .a {color:#F0E6D8; font-size:13px; flex:1;}
        .rp-item .m {font-weight:800; font-size:13px;}
        .rp-progress-wrap {margin-top:10px; background:#0D0910; border-radius:999px; padding:4px;}
        .rp-progress-bar {height:12px; border-radius:999px; background: linear-gradient(90deg,#8B0000,#C4922A,#FFD060,#4DFF99);}
        @media (max-width: 1100px) {
            .rp-two, .rp-poles, .rp-bottom {grid-template-columns:1fr;}
            .rp-grid10 {grid-template-columns: repeat(2, 1fr);}
            .rp-grid6 {grid-template-columns: repeat(2, 1fr);}
            .rp-rails {grid-template-columns:1fr 1fr;}
            .rp-node:before, .rp-node:after {display:none;}
        }
        </style>
        """, unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            <div class="rp-wrap">
                <div class="rp-band">
                    <div class="k">Capital total pilotable • Cockpit patrimonial</div>
                    <div class="v">{_fe(objectif_capital)} €</div>
                    <div class="s">Référence figée fin 2026 • Construction de la phase 0</div>
                </div>

                <div class="rp-two">
                    <div class="rp-card green center">
                        <div class="k">Budget jour / reste du mois</div>
                        <div class="v" style="color:#4DFF99;">{budget_jour} €</div>
                        <div class="s">par jour • reste ce mois : <b>{reste_mois} €</b> sur {jours_restants} jours</div>
                    </div>
                    <div class="rp-card gold center">
                        <div class="k">Complément capital à fournir</div>
                        <div class="v" style="color:#FFD060;">{_fe(complement_capital)} €</div>
                        <div class="s">cible 2 500 € • revenus actuels : {_fe(revenus_total)} € / mois</div>
                    </div>
                </div>

                <div class="rp-node">
                    <div class="k">Nœud central</div>
                    <div class="v">Distribution entre revenus, rente et poches capital</div>
                </div>

                <div class="rp-poles">
                    <div class="rp-pole rev">
                        <div class="k">Revenus mensuels</div>
                        <div class="v" style="color:#4DFF99;">{_fe(revenus_total)} €</div>
                        <div class="rp-grid6">
            """).strip(),
            unsafe_allow_html=True,
        )
        ...
        rev_html = ""
        for nom, valeur in revenus_items:
            color = "#4DFF99" if float(valeur or 0) > 0 else "#7C6A5A"
            rev_html += f'<div class="rp-mini rev"><div class="k">{nom}</div><div class="v" style="color:{color};">{_fe(valeur)} €</div></div>'

        cap_html = ""
        for nom, valeur in poches_items:
            color = "#FFD060" if float(valeur or 0) > 0 else "#7C6A5A"
            cap_html += f'<div class="rp-mini cap"><div class="k">{nom}</div><div class="v" style="color:{color};">{_fe(valeur)} €</div></div>'
        ...
        st.markdown(
            textwrap.dedent(
                ...
                + f"""
                    ...
                    <div class="rp-pole pioche">
                        <div class="k">Rente / pioche capital</div>
                        <div class="v" style="color:#FFD060;">{_fe(complement_capital)} €</div>
                        <div class="s" style="margin-bottom:10px;">ARVA théorique : {_fe(rente_arva)} € / mois</div>
                        <div class="rp-grid10">
                            {cap_html}
                        </div>
                    </div>
                ...
                <div class="rp-rails">
                    <div class="rp-rail parents">
                        <div class="k">Pour mes Parents d'Amour</div>
                        <div class="v" style="color:#FFB3B3;">{parents_amour} €</div>
                    </div>
                    <div class="rp-rail plancher">
                        <div class="k">Plancher</div>
                        <div class="v" style="color:#FF7777;">{rail_plancher} €</div>
                    </div>
                    <div class="rp-rail cible">
                        <div class="k">Cible</div>
                        <div class="v" style="color:#FFD060;">{rail_cible} €</div>
                    </div>
                    <div class="rp-rail bonus">
                        <div class="k">Bonus</div>
                        <div class="v" style="color:#4DFF99;">{rail_bonus} €</div>
                    </div>
                </div>

                <div class="rp-bottom">
                    <div class="rp-card">
                        <div class="k">Prochains mouvements</div>
                        <div class="rp-list">
            """.strip()
            ),
            unsafe_allow_html=True,
        ...
        next_html = ""
        for d, a, m, sens in prochains:
            color = "#4DFF99" if str(m).startswith("+") else ("#FF7777" if str(m).startswith("-") else "#BBA888")
            next_html += f'<div class="rp-item"><div class="d">{d}</div><div class="a">{a}</div><div class="m" style="color:{color};">{m}</div></div>'

        progress_width = max(6, min(100, pct_objectif))
        ...
        st.markdown(
            textwrap.dedent(
                ...
                + f"""
                    ...
                    <div class="rp-card center {('green' if pct_objectif >= 80 else 'gold' if pct_objectif >= 35 else 'red')}">
                        <div class="k">Objectif 15 janvier 2027</div>
                        <div class="v" style="color:#F0E6D8;">{_fe(capital_reel)} €</div>
                        <div class="s">sur {_fe(objectif_capital)} € • {pct_objectif}% atteint</div>
                        <div class="rp-progress-wrap"><div class="rp-progress-bar" style="width:{progress_width}%;"></div></div>
                        <div class="s" style="margin-top:12px;">Compte à rebours : <b>{compte_rebours} jours</b></div>
                        <div class="s">Compte courant : <b style="color:{cc_border};">{_fe(cc_val)} €</b></div>
                    </div>
                </div>
            </div>
            """.strip()
